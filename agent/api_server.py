...
class GeneratePdfRequest(BaseModel):
    ...
    title: str = Field("Research Report", max_length=200)
...
async def session_events(
    ...
    if replay_active and not event_id and session.last_attempt_id:
        ...
        if replay_all:
            replay_attempt_id = session.last_attempt_id
...
def _render_pdf_reportlab(title: str, content: str) -> bytes:
    ...
    emoji_pattern = re.compile(
        "|".join(re.escape(emoji) for emoji in sorted(emoji_replacements, key=len, reverse=True))
    )
    ...
    def replace_emoji(value: str) -> str:
        rendered = emoji_pattern.sub(
            lambda match: (
                ...
                f'[{emoji_replacements[match.group(0)][0]}]</font>'
            ),
            value,
        )
    ...
    def inline_markdown(value: str) -> str:
        ...
        rendered = replace_emoji(rendered)
...
async def generate_response_pdf(payload: GeneratePdfRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=501, detail="Markdown rendering dependency is unavailable") from exc

    safe_title = html.escape(payload.title.strip() or "Research Report")
    safe_markdown = html.escape(payload.content)
    emoji_colors = {
        "🔴": "red", "🟥": "red", "❌": "red", "⛔": "red", "🚨": "red",
        "🟢": "green", "🟩": "green", "✅": "green",
        "🟡": "yellow", "🟨": "yellow", "⚠️": "yellow", "⚠": "yellow",
        "🔵": "blue", "🟦": "blue", "ℹ️": "blue", "ℹ": "blue",
    }
    emoji_pattern = re.compile(
        "|".join(re.escape(emoji) for emoji in sorted(emoji_colors, key=len, reverse=True))
    )
    safe_markdown = emoji_pattern.sub(
        lambda match: (
            f'<span class="emoji emoji-{emoji_colors[match.group(0)]}">'
            f"{match.group(0)}</span>"
        ),
        safe_markdown,
    )
    safe_markdown = re.sub(
        r"(?<![\w>])([\U0001F300-\U0001FAFF])",
...
body {{ font-family: "Microsoft YaHei", "Noto Sans CJK SC", sans-serif; color: #172033; font-size: 10.5pt; line-height: 1.65; }}
h1 {{ font-size: 20pt; color: #111827; border-bottom: 2px solid #2563eb; padding-bottom: 8px; }}
h2 {{ font-size: 15pt; margin-top: 22px; }} h3 {{ font-size: 12pt; }}
...
.emoji-red {{ color: #dc2626; }} .emoji-green {{ color: #16a34a; }}
.emoji-yellow {{ color: #d97706; }} .emoji-blue {{ color: #2563eb; }}
</style>
</head>
<body><h1>{safe_title}</h1><div class="meta">Generated by Vibe-Trading · {datetime.now().strftime("%Y-%m-%d %H:%M")}</div>{body}</body>
</html>"""
    try:
    ...
    filename = re.sub(r"[^A-Za-z0-9_-]+", "_", payload.title.strip())[:80].strip("_") or "research_report"
    return Response(
        content=pdf,
