    ...
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">{t("localAccess")}</h2>
        </div>
        <p className="text-sm text-muted-foreground">{t("localAccessDesc")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{t("serverApiKey")}</span>
          <input
            type="password"
            value={localApiKey}
            onChange={(event) => setLocalApiKeyState(event.target.value)}
            className={fieldClass}
            placeholder={t("browserOnly")}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{t("browserOnly")}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{t("settingsSubtitle")}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? t("oauth", { command: selectedProvider.login_command })
        : t("noKey");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{t("llmDesc")}</p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{t("providerHint")}</span>
            </label>
            ...
            <label className="grid gap-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t("useDefaults")}</span>
                </button>
              </div>
              <span className={hintClass}>{t("modelHint")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{t("baseUrl")}</span>
              <input
                value={form.base_url}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? "OAuth" : t("apiKey")}
              </span>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("timeout")}</span>
              <input
                type="number"
                min={1}
                max={3600}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("maxRetries")}</span>
              <input
                type="number"
                min={0}
                max={20}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("reasoning")}</span>
              <select
              ...
              <span className={hintClass}>{t("reasoningHint")}</span>
            </label>

            <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{t("savedTo")}: </span>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("dataSettings")}</h2>
          </div>
          <p className="text-sm text-muted-foreground">{t("dataDesc")}</p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{t("tushareHint")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
          ...
          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? t("loaderAvailable") : t("noLoader")}
              </span>
            </div>
