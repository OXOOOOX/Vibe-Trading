...
状态：第一阶段开发闭环完成；第二阶段 M7 开发完成，等待影子运行验收
范围：飞书一键触发、全持仓刷新、个股日报、分区审查、组合决策、PDF 交付<br>
...
- [x] 仅交易日触发。
- [x] 与 09:10 预热衔接；预热失败或超时自动切换为强制补刷新。
- [x] 使用 SQLite 持久化运行账本，每个市场日期只自动创建一次。
- [x] 服务重启后恢复中断任务，并通过投递状态机避免重复发送。
- [x] 自动运行失败时发送简短失败卡，允许手动重试。
- [ ] 完成 3–5 个交易日影子运行验收后开启正式投递。

代码默认配置保持关闭；开始影子运行时设置：

```dotenv
VIBE_TRADING_PORTFOLIO_AUTO_RUN_ENABLED=1
VIBE_TRADING_PORTFOLIO_AUTO_RUN_MODE=shadow
```

正式投递改为 `MODE=deliver`。飞书目标默认记忆最近一次手工“组合晨会”的会话；也可以通过
`VIBE_TRADING_PORTFOLIO_AUTO_RUN_CHAT_ID` 显式覆盖。
